...
function AdminTestimonialsPage() {
  const { data: testimonials = [], isLoading, isRefetching, refetch } = useGetAllTestimonials();
  ...
  const handleImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      try {
        const url = await uploadMutation.mutateAsync({ image: base64, folder: "realdreams/testimonials" });
        setForm((prev) => ({ ...prev, avatarUrl: url }));
        setSuccessToast("Avatar photo uploaded successfully!");
        setTimeout(() => setSuccessToast(""), 3500);
      } catch (err: any) {
        setErrorToast(err.message || "Failed to upload avatar image.");
      } finally {
  ...
  const handleToggleStatus = async (item: TestimonialData) => {
    try {
      await updateMutation.mutateAsync({
        id: item.id,
        input: { isActive: !item.isActive },
      });
      setSuccessToast(`Testimonial by "${item.name}" set to ${!item.isActive ? "Active" : "Inactive"}.`);
      setTimeout(() => setSuccessToast(""), 3500);
    } catch (err: any) {
  ...
  const handleStartEdit = (item: TestimonialData) => {
    setEditingItem(item);
    setForm({
      name: item.name,
      role: item.role,
    ...
    setImagePreview(item.avatarUrl || "");
    setShowModal(true);
  };

  const handleDeleteItem = async (item: TestimonialData) => {
    if (!confirm(`Delete testimonial by "${item.name}"? This action cannot be undone.`)) return;
    try {
      await deleteMutation.mutateAsync(item.id);
      setSuccessToast(`Testimonial by "${item.name}" deleted.`);
      setTimeout(() => setSuccessToast(""), 3500);
    } catch (err: any) {
      setErrorToast(err.message || "Failed to delete testimonial.");
    }
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim()) {
      setErrorToast("Please enter client/person name.");
      return;
    }
    if (!form.role.trim()) {
      setErrorToast("Please enter client role or designation.");
      return;
    }
    if (!form.quote.trim()) {
      setErrorToast("Please enter testimonial content/quote.");
      return;
    }
    ...
    try {
      if (editingItem) {
        ...
        setSuccessToast(`Testimonial by "${form.name}" updated successfully!`);
      } else {
        await createMutation.mutateAsync(form);
        setSuccessToast(`Testimonial by "${form.name}" added successfully!`);
      }
      setShowModal(false);
      setEditingItem(null);
      setForm({
      ...
      setErrorToast(err.message || "Failed to save testimonial.");
    }
  };
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Add, update, or remove client reviews displayed on the website homepage.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-4 rounded-xl border border-border bg-card">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground border-t sm:border-t-0 pt-3 sm:pt-0">
          <div>
            Total: <span className="font-bold text-foreground">{testimonials.length}</span>
          </div>
          <div>
            Active:{" "}
      ...
      {isLoading ? (
        ...
        <div className="p-12 text-center bg-card rounded-xl border border-border space-y-3">
          <Quote className="h-10 w-10 text-muted-foreground/40 mx-auto" />
          <h3 className="font-bold text-foreground">No testimonials found</h3>
          <p className="text-sm text-muted-foreground max-w-sm mx-auto">
            {searchTerm ? "No testimonial matches your search criteria." : "No testimonials have been created yet."}
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-muted/50 text-xs uppercase tracking-wider text-muted-foreground border-b border-border">
                <tr>
                  ...
                  <th className="px-4 py-3.5">Testimonial Quote</th>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        {item.avatarUrl ? (
                          ...
                          <div className="h-10 w-10 rounded-full bg-ember/10 text-ember font-bold flex items-center justify-center border border-ember/20">
                            {item.name.charAt(0)}
                          </div>
                        )}
                        <div>
                    ...
                    <td className="px-4 py-4">
                      ...
                      {item.company && (
                        <div className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                          <Building className="h-3 w-3" /> {item.company}
                        </div>
                      )}
                    </td>
          ...
          <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-5 border-b border-border bg-muted/30">
              <h3 className="font-bold text-lg text-foreground flex items-center gap-2">
                <MessageSquareQuote className="h-5 w-5 text-ember" />
            ...
            <form onSubmit={handleFormSubmit} className="p-5 space-y-4 overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. Vikrant Pandey"
                    value={form.name}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. Metro Infra Build"
                    value={form.company}
